...
//어노테이션(Annotation) : API 문서에 특별한 설명을 제공하기 위해 만드어진 기능
...
@WebServlet(value="/first.itwill")
public class FirstServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
	
	//클라이언트 요청에 의해 자동 호출되는 메소드 - 요청 처리 메소드
	//=> 클라이언트 요청에 대한 처리 명령과 응답결과를 저장한 파일을 생성하는 명령 작성
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		//클라이언트에게 요청에 대한 실행결과를 파일로 제공하기 위한 파일 형태와 응답파일을 생성하기 위한
		//출력스트림을 반환받아 저장
		//=>클라이언트에게 URL 주소로 응답할 경우 응답파일로 제공하지 않으므로 생략가능
